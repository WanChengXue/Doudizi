...
Card2Column = {5: 0, 6: 1, 7: 2, 8: 3, 9: 4, 10: 5, 11: 6, 12: 7, 13: 8, 14: 9, 17: 10}
    ...
    def _get_reward(self):
        ...
        if winner == "landlord":
            if self.is_chuantian():
                landlord_reward = (
                    self.jiaodizhu_beishu * self.dipai_beishu * (2.0**bomb_num) * 2
                )
            else:
                landlord_reward = (
                    self.jiaodizhu_beishu * self.dipai_beishu * (2.0**bomb_num)
                )
...
def _get_obs_landlord(infoset):
    ...
    farmer_num_cards_left_batch = np.repeat(
        farmer_num_cards_left[np.newaxis, :], num_legal_actions, axis=0
    )

    # --- 农民出过的牌进行编码 ---
    farmer_played_cards = _cards2array(infoset.played_cards["farmer"])
    farmer_played_cards_batch = np.repeat(
        farmer_played_cards[np.newaxis, :], num_legal_actions, axis=0
    )
    ...
    z_batch = np.repeat(
        z[np.newaxis, :, :], num_legal_actions, axis=0
    )  # 重复一下，变成action_dim * 5 * 138的矩阵
    obs = {
        "position": "landlord",
        "x_batch": x_batch.astype(np.float32),  # 这个返回的是带有所有可能动作构成的0-1矩阵action_dim * 260
        "z_batch": z_batch.astype(
            np.float32
        ),  # 这个是历史的15次move构成的5*138的0-1矩阵，在action维度上进行了复制
        "legal_actions": infoset.legal_actions,  # 这个是所有可能的动作构成的列表
        "x_no_action": x_no_action.astype(
            np.int8
        ),  # 这个是不带所有动作构成的0-1矩阵，action_dim * 214
