...
class data_receiver_server(base_server):
    ...
    def __init__(self, args):
        ...
        self.read_data_from_local_machine = self.policy_config.get('read_data_from_local_machine', False)
    ...
    def _parse_server_id_and_connect_server(self):
        ...
        logger_Path = pathlib.Path(self.config_dict['log_dir'] + '/dataserver_log/{}_{}/{}'.format(self.local_rank, self.policy_name, self.data_server_local_rank))
        self.logger = setup_logger('DataServer_log_{}_{}'.format(self.local_rank, self.data_server_local_rank),logger_Path)
        self.logger.info("-------------- 该数据进程使用的机器id为: {}, 对应的设备的索引为: {}, 数据进程的索引为: {}"
            .format(self.machine_index, self.local_rank, self.data_server_local_rank))
        if not self.read_data_from_local_machine:
        ...
        # ------------- 这个地方链接上共享内存服务 -------------------------
        plasma_id = generate_plasma_id(self.machine_index, self.local_rank, self.data_server_local_rank)
        self.plasma_data_id = plasma.ObjectID(plasma_id)
        if self.priority_replay_buffer:
            weight_plamsa_id = generate_plasma_id_for_PEB(self.machine_index, self.local_rank, self.data_server_local_rank)
            self.weight_plasma_id = plasma.ObjectID(weight_plamsa_id)
        # -------------- 将这个服务链接到plasma共享内存服务，需要提前开启，给两次链接机会 ------------
        self.plasma_client = plasma.connect("{}".format(self.policy_config['plasma_server_location']), 2)

        self.logger.info("------------- 创建dataserver成功, worker端链接的地址为:{} -------------".format("tcp://{}:{}".format(self.server_ip, self.server_port)))
        if self.start_data_saved:
            # ------- 只有要将数据保存到本地的开关打开的时候，才需要创建一个dataserver的保存路劲 ---------
            self.data_saved_path = self.config_dict['data_saved_folder'] + '/' + '{}/{}_{}/{}'.format(self.policy_name, self.machine_index, self.local_rank, self.data_server_local_rank)
            create_folder(self.data_saved_path)
            self.logger.info("------------- dataserver接收来自于worker端的数据, 保存的位置为:{} -----------------".format(self.data_saved_path))

    def _load_data_from_memory(self):
        # ------------------ 定义数据保存的路径 -----------------
        self.data_folder = self.config_dict['data_saved_folder'] + '/' + '{}/{}_{}/{}'.format(self.policy_name, self.machine_index, self.local_rank, self.data_server_local_rank)
        # ------------------ 定义权重保存的路径 ------------------
        self.weight_folder = self.config_dict['weight_saved_folder'] + '/' + '{}/{}_{}/{}'.format(self.policy_name, self.machine_index, self.local_rank, self.data_server_local_rank)
        data_file_list = os.listdir(self.data_folder)
        self.data_path_list = []
        for file_name in data_file_list:
            self.data_path_list.append(file_name)
        self.sample_file_number = self.policy_config['batch_size'] // self.policy_config['woker_transmit_interval']

    def _save_data(self, frame_data):
        # ------ 这个函数是将接收到的数据保存到本地 ---------- 
        # -- 构建保存路径 ----
        saved_path = self.data_saved_path + '/{}_pickle_data.pickle'.format(self.data_receive_count)
        ...
        if self.receiver in socket:
            ...
            if self.corporative_scenario:
                ...
                for key in self.replay_buffer:
                    self.logger.info("------------- 本次接收数据数目为：{},此时buffer {} 中的数据为:{} ------------".format(current_sample_recv_number, key, self.replay_buffer[key].buffer_size))
            if len(raw_data_list) > 0:
    ...
    def _sample_data_from_replay_buffer(self):
        ...
        if self.corporative_scenario:
            ...
            if self.priority_replay_buffer:
                ...
                for key in self.replay_buffer:
                    agent_sample_data_dict, agent_sample_index, agent_weight_data = self.replay_buffer[key].sample_data()
        ...
        if self.priority_replay_buffer:
            ...
            if self.corporative_scenario:
                ...
                for key in weight_data:
                    self.replay_buffer[key].update_priorities(self.sample_index[key], weight_data[key])
            # --------- 将plasma client中权重id的数据删除 --------
            self.plasma_client.delete([self.weight_plasma_id])
            self.logger.info("================== 更新完成priority replay buffer中的数据样本优先级 =========")
