        ...
        if "time" in self.tag_func_dict[tag]:
            ...
            if len(self.tag_values_dict[tag]) >= self.tag_output_threshold_dict[tag]:
                if self.tag_func_dict[tag] == "sum":
                    ...
                    output_value = sum(self.tag_values_dict[tag]) / len(
                        self.tag_values_dict[tag]
                    )
                ...
                self.summarywriter.add_scalar(
                    tag, output_value, self.tag_step_dict[tag]
                )
                self.tag_step_dict[tag] += 1
                self.tag_values_dict[tag] = []
...
class log_server(base_server):
    def __init__(self, config_path):
        ...
        self.total_data_server_per_machine = (
            self.policy_config["device_number_per_machine"]
            * self.policy_config["server_number_per_device"]
        )
        self.log_receiver = self.context.socket(zmq.PULL)
        self.log_receiver.bind(
            "tcp://{}:{}".format(
        ...
        self.logger.info(
            "----------- 完成log server的构建，数据保存到tensorboard的路径为：{} ---------------".format(
                self.policy_config["tensorboard_folder"]
            )
        )
        # ----- 打开tensorboard服务，将输出结果和错误信息丢掉，本地打开训练机器的tensorboard，直接网页上打开,tensorboard默认6006端口，ip:6006 ---------
        self.open_tensorboard_server()
        create_folder(
            self.config_dict["policy_config"]["tensorboard_folder"], delete_origin=True
        )
        self.logger.info(
            "================== 完成log server的构建，配置好了tensorboard的路径为 {}".format(
                self.config_dict["policy_config"]["tensorboard_folder"]
            )
        )
    ...
    def open_tensorboard_server(self):
        ...
        tensorboard_command = "nohup python -m tensorboard.main --logdir={} --host={} > /dev/null 2>&1 &".format(
            abs_path, server_ip
        )
        os.system(tensorboard_command)
        self.logger.info(
            "----------------- 创建好了tensorboard日志文件，命令是{} -----------------".format(
                tensorboard_command
            )
        )
    ...
    def summary_definition(self):
        ...
        self.summary_logger.add_tag(
            "result/accumulated_reward/{}".format(self.policy_name), 1, "mean"
        )
        # =============== 采样端的指标，请求模型的时间，更新模型的时间，从configserver下载模型的时间，完整的采样一条轨迹需要的时间 ===============
        self.summary_logger.add_tag(
            "sampler/episode_time/{}".format(self.policy_name), 10, "mean"
        )
        self.summary_logger.add_tag(
            "sampler/model_request_time/{}".format(self.policy_name), 10, "mean"
        )
        self.summary_logger.add_tag(
            "sampler/model_update_interval/{}".format(self.policy_name), 10, "mean"
        )
        self.summary_logger.add_tag(
            "sampler/http_model_download_time/{}".format(self.policy_name), 10, "mean"
        )
        # =============== 定义dataserver的相关参数， 每分钟接收到的数据量，每分钟解析数据的时间，每分钟解析套接字的时间，从trainingset转移到plasma client的时间。以及采样worker的数目 =============
        self.summary_logger.add_tag(
            "data_server/dataserver_recv_instance_per_min/{}".format(self.policy_name),
            self.total_data_server_per_machine,
            "sum",
        )
        self.summary_logger.add_tag(
            "data_server/dataserver_parse_time_per_min/{}".format(self.policy_name),
            1,
            "sum",
        )
        self.summary_logger.add_tag(
            "data_server/dataserver_socket_time_per_min/{}".format(self.policy_name),
            1,
            "mean",
        )
        self.summary_logger.add_tag(
            "data_server/dataserver_sampling_time_per_min/{}".format(self.policy_name),
            1,
            "mean",
        )
        self.summary_logger.add_tag("Worker/active_worker", 1, "sum")
        self.summary_logger.add_tag(
            "Worker/sample_step_per_episode/{}".format(self.policy_name), 10, "mean"
        )
        # =============== 定义策略相关的指标，包括loss，等等 =========================
        self.summary_logger.add_tag(
    ...
    def run(self):
        ...
        while True:
            if time.time() > self.next_calculate_worker_time:
                ...
                self.summary_logger.add_summary(
                    "Worker/active_worker", len(self.active_worker)
                )
                self.active_worker = {}
