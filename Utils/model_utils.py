    ...
    for agent_name in net_dict.keys():
        if isinstance(net_dict[agent_name], dict):
            ...
            for model_type in net_dict[agent_name].keys():
                ...
                torch.save(
                    net_dict[agent_name][model_type].state_dict(), model_save_path
                )
                remove_old_version_model(
                    model_path_prefix + "/" + agent_name + "_" + model_type + "*",
                    cache_size,
                    logger,
                )
            ...
            remove_old_version_model(
                model_path_prefix + "/" + agent_name + "*", cache_size, logger
            )
...
def create_model(model_config):
    ...
    model_fn = getattr(
        importlib.import_module("Model.{}".format(model_name)),
        "init_{}_{}_net".format(model_config["model_type"], model_config["agent_name"]),
    )
