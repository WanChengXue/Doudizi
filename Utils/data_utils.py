        ...
        with self._lock:
            for data_point in recv_data:
                if self.current_buffer_length < self._capacity:
                    ...
                    self.buffer[
                        self.current_buffer_length % self._capacity
                    ] = data_point
    ...
    def _sample_proportional(self):
        ...
        for _ in range(self._batch_size):
            mass = random.random() * self._it_sum.sum(
                0, self.current_buffer_capcity - 1
            )
...
def convert_data_format_to_torch_trainig(training_batch, local_rank):
    ...
    for key, value in training_batch.items():
        if isinstance(value, dict):
            training_batch[key] = convert_data_format_to_torch_trainig(
                value, local_rank
            )
        else:
...
def convert_list_to_dict(obs_list):
    ...
    if isinstance(instance_sample, dict):
        ...
        for key in instance_sample:
            if key == "actions":
                sub_obs[key] = np.hstack(
                    convert_list_to_dict(
                        [single_sample[key] for single_sample in obs_list]
                    )
                )
            elif key == "next_state_action_length":
                sub_obs[key] = np.array(
                    [single_sample[key] for single_sample in obs_list]
                )
            else:
                # -------- 将这个子字典中的所有key获取到 -------
                sub_obs[key] = convert_list_to_dict(
                    [single_sample[key] for single_sample in obs_list]
                )
        ...
        if isinstance(instance_sample[0], dict):
            ...
            sub_obs = [
                convert_list_to_dict(zip_object) for zip_object in zip(*obs_list)
            ]
...
def squeeze(feature):
    ...
    if isinstance(feature, dict):
        return [value for key in sorted(feature) for value in squeeze(feature[key])]
    elif isinstance(feature, (tuple, list, set)):
        return [value for item in feature for value in squeeze(item)]
...
class OUNoise:
    ...
    def step(self, current_actions):
        ...
        ou_explore_action = current_actions + self.ou_epsilon * torch.FloatTensor(
            self.state
        ).unsqueeze(0)
...
def GAE_estimator(data_dict, gamma, tau, bootstrap_value, multiagent_scenario):
    if multiagent_scenario:
        ...
        for agent_name in data_dict.keys():
            GAE_estimator_single_agent(
                data_dict[agent_name], gamma, tau, bootstrap_value[agent_name]
            )
