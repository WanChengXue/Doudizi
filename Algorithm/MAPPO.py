...
class MAPPOTrainer:
    def __init__(self, model, optimizer, scheduler, policy_config):
        ...
        self.advantage_normalization = self.policy_config.get(
    ...
    def update_centralized_critic(
        self, target_state_value, centralized_state, old_state_value
    ):
        ...
        if self.clip_value:
            ...
            value_pred_clipped = old_state_value + (
                predict_state_value - old_state_value
            ).clamp(-value_clamp_range, value_clamp_range)
            if self.use_popart:
                pass
            else:
                clipped_state_value_loss = self.critic_loss(
                    target_state_value, value_pred_clipped
                )
                unclipped_state_value_loss = self.critic_loss(
                    target_state_value, predict_state_value
                )
                value_loss_vecotr = torch.max(
                    clipped_state_value_loss, unclipped_state_value_loss
                )
            ...
            if self.use_popart:
                ...
                value_loss_vecotr = self.critic_loss(
                    target_state_value, predict_state_value
                )
        ...
        value_loss.backward()
        centralized_critic_grad_norm = torch.nn.utils.clip_grad_norm_(
            self.model["centralized_critic"].parameters(), self.max_grad_norm
        )
    ...
    def update_policy(
        self, agent_name, agent_obs, agent_action, agent_old_log_prob, agent_advantage
    ):
        policy_info_dict = dict()
        # self.test_current_model(agent_name)
        # -------- 使用当前的policy网络计算一下action的log值，以及这个动作的entropy -------
        try:
            action_log_probs, entropy = self.model[agent_name][
                "policy"
            ].module.evaluate(agent_obs, agent_action)
        except:
            action_log_probs, entropy = self.model[agent_name]["policy"].evaluate(
                agent_obs, agent_action
            )
        # ------ 计算重要性因子 -------
        importance_ratio = torch.exp(action_log_probs - agent_old_log_prob)
        surr1 = importance_ratio * agent_advantage
        policy_info_dict["Surr1"] = surr1.mean().item()
        # ----------- 使用了PPO算法，这里需要进行clip操作 ---------
        surr2 = (
            torch.clamp(
                importance_ratio, 1.0 - self.clip_epsilon, 1.0 + self.clip_epsilon
            )
            * agent_advantage
        )
        ...
        if self.dual_clip is not None:
            # ---------- 使用双梯度剪裁，默认开启 -----------
            surr3 = torch.min(
                self.dual_clip * agent_advantage, torch.zeros_like(agent_advantage)
            )
    ...
    def update_critic(self, agent_name, agent_obs, old_state_value, target_state_value):
    ...
    def test_current_model(self, agent_name):
        ...
        for name, value in self.model[agent_name]["policy"].named_parameters():
            if value.requires_grad:
                print(
                    torch.sum(
                        self.data["current_parameter_dict"][
                            "actor." + name.split("module.", 1)[-1]
                        ].data
                        - value.data
                    )
                )
    ...
    def test_grad_and_new_model(self, agent_name):
        ...
        for name, value in self.model[agent_name]["policy"].named_parameters():
            if value.requires_grad:
                print(
                    torch.sum(
                        self.data["policy"]["grad"][name.split("module.", 1)[-1]].to(0)
                        - value.grad
                    )
                )
                # # ---------- 测试新模型 ------------
    ...
    def step(self, training_data):
        info_list = []
        # import pickle
        # open_file = open('training_log.pickle', 'rb')
        ...
        for ep in range(self.ppo_update_epoch):
            ...
            if self.use_centralized_critic:
                ...
                self.update_centralized_critic(
                    target_state_value, current_centralized_state, old_state_value
                )
                if self.advantage_normalization:
                    centralized_advantage_value_mean = torch.mean(
                        centralized_advantage_value
                    )
                    centralized_advantage_value_std = torch.std(
                        centralized_advantage_value
                    )
                    centralized_advantage_value = (
                        centralized_advantage_value - centralized_advantage_value_mean
                    ) / centralized_advantage_value_std

            for agent_name in self.agent_name_list:
                # training_data[agent_name] = dict()
                # training_data[agent_name]['current_agent_obs'] = self.data['current_state'].to(0)
                # training_data[agent_name]['actions'] = self.data['actions'].to(0)
                # training_data[agent_name]['old_log_prob'] = self.data['log_log_prob'].to(0)
                # training_data[agent_name]['return'] = self.data['return'].to(0).unsqueeze(-1)
                # training_data[agent_name]['old_obs_value'] = None
                # training_data[agent_name]['advantages'] = self.data['advantage'].to(0)
                if self.use_centralized_critic:
                    ...
                    agent_return = training_data[agent_name]["target_state_value"]
                    self.update_critic(
                        agent_name, agent_obs, agent_old_state_value, agent_return
                    )
                    if self.advantage_normalization:
                        agent_advantage_mean = torch.mean(agent_advantage)
                        agent_advantage_std = torch.std(agent_advantage)
                        agent_advantage = (
                            agent_advantage - agent_advantage_mean
                        ) / agent_advantage_std
                # ------------- actor update ---------------
                self.update_policy(
                    agent_name,
